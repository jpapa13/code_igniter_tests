...
abstract class REST_Controller extends \CI_Controller {
    ...
    protected function _auth_override_check()
    {
        ...
        if ( ! empty($auth_override_class_method))
        {
            ...
            if ( ! empty($auth_override_class_method[$this->router->class]['*'])) // Check for class overrides
            {
                ...
                // Basic auth override found, prepare bearer
                if ($auth_override_class_method[$this->router->class]['*'] === 'bearer')
                {
                    $this->_prepare_bearer_auth();
            ...
            if ( ! empty($auth_override_class_method[$this->router->class][$this->router->method]))
            {
                ...
                // Basic auth override found, prepare bearer
                if ($auth_override_class_method[$this->router->class][$this->router->method] === 'bearer')
                {
                    $this->_prepare_bearer_auth();
        ...
        if ( ! empty($auth_override_class_method_http))
        {
            ...
            if ( ! empty($auth_override_class_method_http[$this->router->class]['*'][$this->request->method]))
            {
                ...
                // Basic auth override found, prepare bearer
                if ($auth_override_class_method_http[$this->router->class]['*'][$this->request->method] === 'bearer')
                {
                    $this->_prepare_bearer_auth();
            ...
            if ( ! empty($auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method]))
            {
                ...
                // Basic auth override found, prepare bearer
                if ($auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] === 'bearer')
                {
                    $this->_prepare_bearer_auth();
     ...
     * @return void
     */
    protected function _prepare_bearer_auth()
    {
        // If whitelist is enabled it has the first chance to kick them out
        if ($this->config->item('rest_ip_whitelist_enabled'))
        {
        ...
        // Returns NULL if the SERVER variables PHP_AUTH_USER and HTTP_AUTHENTICATION don't exist
        $authorization = $this->input->server('Authorization');
        ...
        if ($http_auth !== NULL)
        {
            // If the authentication header is set as bearer, then extract the username and password from
            // HTTP_AUTHORIZATION e.g. my_username:my_password. This is passed in the .htaccess file
            if (strpos(strtolower($http_auth), 'bearer') === 0)
            {
                ...
                {
                    $this->load->library($auth_library_class);
                    if(!$this->{$auth_library_class}->$auth_library_function($http_auth)){
                        // Display an error response
                        $this->response([
                            'status' => FALSE,
                            'data' => $this->lang->line('text_rest_invalid_credentials')
                        ], self::HTTP_UNAUTHORIZED);
                    }
                }else{
                    $this->_force_login();
